...
#include "ui_zglobycoincontroldialog.h"
...
std::list<std::string> ZGlobycoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGlobycoinControlDialog::listMints;

ZGlobycoinControlDialog::ZGlobycoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGlobycoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGlobycoinControlDialog::~ZGlobycoinControlDialog()
{
    delete ui;
}

void ZGlobycoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGlobycoinControlDialog::updateList()
{
...
void ZGlobycoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGlobycoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGlobycoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGlobycoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGlobycoinControlDialog::GetSelectedMints()
{
...
void ZGlobycoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
