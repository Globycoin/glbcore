...
#include "zglobycoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGLB ought to be enough for anybody." - Bill Gates, 2017
    ui->zGLBpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGLBSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzGLB->setEnabled(false);
        ui->pushButtonMintzGLB->setToolTip(tr("zGLB is currently disabled due to maintenance."));

        ui->pushButtonSpendzGLB->setEnabled(false);
        ui->pushButtonSpendzGLB->setToolTip(tr("zGLB is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGLBpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGLB_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGLB is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGLB...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGLB in ") +
...
void PrivacyDialog::on_pushButtonSpendzGLB_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGLB is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zGLB
        sendzGLB();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGLB
    sendzGLB();
}

void PrivacyDialog::on_pushButtonZGlobycoinControl_clicked()
{
    ZGlobycoinControlDialog* zGlobycoinControl = new ZGlobycoinControlDialog(this);
    zGlobycoinControl->setModel(walletModel);
    zGlobycoinControl->exec();
}

void PrivacyDialog::setZGlobycoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGlobycoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGLB()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Globycoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGLBpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGLB is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ART </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGLB</b>";
    ...
    // use mints from zGlobycoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGlobycoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGlobycoinControlDialog::GetSelectedMints();
    }
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zGLB transaction
    ...
    // Clear zglobycoin selector in case it was used
    ZGlobycoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGlobycoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Globycoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGlobycoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGLB </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGLB "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGLB "));
    ui->labelzGLBAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzGLBSyncStatus->setVisible(fShow);
}
