...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
   ...
   CTxOut vout = CTxOut(MASTERNODE_COLLATERAL_LIMIT, obfuScationPool.collateralPubKey);
    ...
    // should be at least not earlier than block when 10000 ART tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 ART tx -> 1 confirmation
