...
#if defined(NDEBUG)
#error "Globycoin cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zGlobycoin spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zGlobycoin spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zGlobycoin spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
    ...
    } else if(nHeight >=1 && nHeight <= 2000) { //PoW phase
		nSubsidy = 1 * COIN;
    } else if(nHeight >2000 && nHeight <= 70000) { //PoS phase
		nSubsidy = 777 * COIN;
    } else if(nHeight >70000 && nHeight <= 250000) { //PoS phase
		nSubsidy = 400 * COIN;
    } else if(nHeight >250000 && nHeight <= 500000) { //PoS phase
		nSubsidy = 380 * COIN;
    } else if(nHeight > 500000 && nHeight <= 750000) { //PoS phase
		nSubsidy = 360 * COIN; 
		...
		nSubsidy = 260 * COIN;
    } else if(nHeight > 2000000 && nHeight <= 2250000) { 
		nSubsidy = 240 * COIN;
    } else if(nHeight > 2250000 && nHeight <= 2500000) { 
		nSubsidy = 220 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Globycoin
...
{
    RenameThread("globycoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZARTMinted()
{
...
void RecalculateZARTSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zART supply
...
bool RecalculateARTSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zGlobycoin with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zGlobycoinSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ART to zART
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zGlobycoin spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zGlobycoin serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zGlobycoin mints and %d zGlobycoin spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Globycoin: We use certain sporks during IBD, so check to see if they are
