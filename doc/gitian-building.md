...
*Setup instructions for a gitian build of Globycoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Globycoin
...
- [Building Globycoin](#building-globycoin)
...
Clone the git repositories for globycoin and gitian and then checkout the globycoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the globycoin repository
...
To build Globycoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the globycoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/globycoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/Globycoin
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Globycoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit globycoin=${COMMIT} --url globycoin=${URL} ../Globycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit globycoin=${COMMIT} --url globycoin=${URL} ../Globycoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit globycoin=${COMMIT} --url globycoin=${URL} ../Globycoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/globycoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/globycoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/globycoin-build.assert
```
